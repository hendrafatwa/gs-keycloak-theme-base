...
<@layout.registrationLayout title="Buat Password Baru"; section>
  <#if section == "form">
    ...
    <h4 class="mb-1 fw-bold">Buat Password Baru</h4>
    <p class="mb-6">Password baru harus berbeda dari password sebelumnya.</p>

    <form id="newPassForm" action="${url.loginAction}" method="post">
    ...
    (function () {
      ...
      function periksa(pwd) {
        return {
          ...
          lower:  /[a-z]/.test(pwd),
          digit:  /[0-9]/.test(pwd),
          // Simbol = apa pun selain huruf dan angka, termasuk spasi.
          // Sengaja tidak dibatasi daftar tertentu supaya user bebas memilih.
