...
<@layout.registrationLayout title="Verifikasi WhatsApp"; section>
  <#if section == "form">
    ...
    <p class="mb-4">Langkah terakhir sebelum membuat password baru.</p>
      ...
      <div class="text-center">
        ...
        <div class="border rounded-2 bg-white p-3 text-start position-relative"
          ...
          <!-- Tombol menyalin SELURUH pesan, bukan hanya baris KEY, supaya
               user yang mengirim manual tidak perlu mengetik ulang. -->
...
>Permintaan Reset Password - ${waAppName!""}

Saya ingin melakukan reset password akun ${waAppName!""}.
KEY: ${waLoginKey!""}
...
Mohon kirimkan kode OTP untuk verifikasi.</pre>
    ...
    <form action="${url.loginAction}" method="post" class="mb-4">
      <input type="hidden" name="rpAction" value="RENEW_WA" />
    ...
    (function () {
      ...
      function updateOtp() {
        ...
        hiddenOtp.value = v;
        if (v.length === 6) {
          setTimeout(() => { showLoading(); form.submit(); }, 300);
        }
      }
      ...
      // Tidak auto-focus ke kolom OTP — langkah pertama user adalah membuka
      // WhatsApp, bukan mengetik. Keyboard yang langsung naik di HP justru
      // menutupi tombolnya.
